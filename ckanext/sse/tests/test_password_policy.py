...
class TestBlocklist:
    def test_the_bundled_list_loads(self):
        assert len(pp._blocklist_file_words()) > 90000

    def test_a_bundled_common_password_is_rejected(self):
        # 12+ chars, so it clears the length rule and is caught by the list
        # rather than by minimum length.
        failures = pp.check_strength("justinbieber")
        assert any("common" in f for f in failures)

    def test_a_passphrase_is_accepted(self):
        assert pp.check_strength("violet tractor mango lantern") == []

    def test_extra_blocklist_files_are_merged(self, tmp_path):
        extra = tmp_path / "extra.txt"
        extra.write_text("zzzuniquebannedword\n")
        with changed_config(
            "ckanext.sse.password.blocklist_files", str(extra)
        ):
            pp._BLOCKLIST_CACHE.clear()
            assert "zzzuniquebannedword" in pp._blocklist_file_words()
        pp._BLOCKLIST_CACHE.clear()
